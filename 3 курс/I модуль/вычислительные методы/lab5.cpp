...
double* methodRungeKutta(int n, double aa, double a, double b, double y0){
    ...
    y[0]=y0;
    for (int i=0;i<n;i++){
        y[i+1] = y[i]+((1-aa)*f(x[i],y[i])+aa*f(x[i]+(h/(2*aa)),(h/(2*aa))*f(x[i],y[i])))*h;
    }
...
int main(){
    double a = 0; double b = 1; int n = 32; double y0 = 1; double aa = 1;
    ...
    double* y = methodRungeKutta(n, aa, a, b, y0); 
    ...
    for (int i=1;i<=100000000;i++){
        ...
        double* y1 = methodRungeKutta(i, aa, a, b, y0); 
